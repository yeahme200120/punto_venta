...
<aside x-data="sidebar()" :class="{
    ...
    <button @click="toggleCollapse()"
        ...
        <div class="p-1 transition-colors bg-indigo-600 rounded-full group-hover:bg-indigo-500">
            <svg :class="collapsed ? 'rotate-180' : ''" class="w-4 h-4 text-white transition-transform duration-300"
    ...
    <nav
        class="flex-1 p-3 space-y-2 overflow-x-hidden overflow-y-auto scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent hover:scrollbar-thumb-white/30">
        @foreach($modulos as $modulo)
                                ...
                                <div x-data="{ openSubmenu: {{ $isMenuActive ? 'true' : 'false' }} }">
                                    <div @click.stop.prevent="openSubmenu = !openSubmenu"
                                        ...
                                        <svg x-show="!collapsed" class="flex-shrink-0 w-4 h-4 transition-transform duration-200"
                                            :class="{'rotate-90': openSubmenu}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7">
                                            </path>
                                        </svg>
                                    </div>

                                    <div x-show="openSubmenu" x-collapse.duration.300ms
        ...
        @endforeach
        @php
            if ($modulo->nombre == 'Cobranza') {
                \Log::info('Usuario autenticado: ' . (auth()->check() ? auth()->user()->email : 'NO'));
                \Log::info('Usuario ID: ' . (auth()->check() ? auth()->id() : 'N/A'));
                \Log::info('=== MÓDULO COBRANZA ===');
                \Log::info('Permiso: ver_cobranza');
                \Log::info('Usuario puede: ' . (auth()->user()->can('ver_cobranza') ? 'SI' : 'NO'));
                \Log::info('Menús count: ' . $modulo->menus->count());
                foreach ($modulo->menus as $menu) {
                    \Log::info('  Menú: ' . $menu->nombre . ' - Hijos: ' . $menu->hijos->count());
                }
            }
        @endphp
    </nav>
    ...
    {{-- Footer - Colapsable --}}
    <div
        class="p-4 mt-auto transition-all duration-300 border-t border-white/20 bg-gradient-to-r from-transparent to-indigo-800/20">
        <div x-show="!collapsed" class="space-y-2 text-xs">
            <div class="flex items-center justify-between">
                <span class="text-blue-200">Licencia:</span>
                <span
                    class="font-medium {{ $empresaActiva->fecha_fin && $empresaActiva->fecha_fin->isPast() ? 'text-red-300' : 'text-white' }}">
